        ...
        for (const columnName in columns) {
          ...
          if (currentAttribute.references) {
            ...
            for (const foreignKeyReference of foreignKeyReferences) {
              ...
              if (!!constraintName && foreignKeyReference.tableCatalog === database && (schema ? foreignKeyReference.tableSchema === schema : true) && foreignKeyReference.referencedTableName === (references.model?.table || references.model) && foreignKeyReference.referencedColumnName === references.key && (schema ? foreignKeyReference.referencedTableSchema === schema : true) && !removedConstraints[constraintName]) {
                console.log('======================constraints removed')
                await this.queryInterface.removeConstraint(tableName, constraintName, options);
                removedConstraints[constraintName] = true;
              }else{
                console.log('======================constraints not removed')
                console.log(foreignKeyReference)
                console.log(references)
                console.log(`${!!constraintName}, ${foreignKeyReference.tableCatalog === database}, ${(schema ? foreignKeyReference.tableSchema === schema : true)}, ${foreignKeyReference.referencedTableName === (references.model?.table || references.model)}, ${foreignKeyReference.referencedColumnName === references.key}, ${(schema ? foreignKeyReference.referencedTableSchema === schema : true)}, ${!removedConstraints[constraintName]}`)
              }
            }
          }
